<template>
  <div>
    <div class="flex items-start justify-between gap-2">
      ...
      <NuxtLink to="/recipes/new" class="flex items-center justify-center w-12 h-12 text-white rounded-full bg-primary"
        @click="">
    ...
    <div v-for="category in recipeStore.groupRecipesByCategory(filteredRecipes)">
      <div class="flex flex-row items-center gap-5">
        <RecipeCategory :category="category"></RecipeCategory>
      ...
      <div class="grid gap-8 lg:grid-cols-4">
        <div v-for="recipe in category.recipes">
          <NuxtLink :to="`/recipes/${recipe.id}`"
            ...
            <img class="absolute object-cover w-full h-full" :src="recipe.header_image ?? FALLBACK_IMAGE"
              alt="main image" />
            <div class="absolute w-full h-full bg-gradient-to-t from-black/75 to-black/0 z-100" />
            <h2 class="z-10 p-5 font-black tracking-wider text-white text-md md:text-xl">
              {{ recipe.name }}
            </h2>
          </NuxtLink>
        </div>
